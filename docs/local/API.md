...
`apps/api/app.ts:211` agent), with the path from `pathOf` (`surface.ts:371`).
...
prefix when a call asks for it (`:76`).
`GET /api/health` (`apps/api/server.ts:181`) is the one route outside the
