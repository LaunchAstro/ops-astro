...
| `task.decide`                                | `gateId`                                 | a string; absent is refused                                                | `refuseMistypedIdentifier`, from `prepareCommand` (`commands/prepare.ts`)                              |
...
The last three rows are `TYPED_IDENTIFIERS` (`commands/prepare.ts`), checked
...
into its own batch or makes the restore refuse `PARENT_TRASHED` 409.
...
form, with one exception below. The database's uuid cast takes either case.
...
one `PROPOSAL_OUT_OF_SCOPE` 403 (`refuseBeyondBudget`,
