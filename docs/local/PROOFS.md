...
`tests/acceptance/` takes the mechanisms the other lanes built and checks,
through the surfaces a caller has, whether they hold. None of it is a plan and
none of it counts declarations. Where a proof could not be written, this file
names it.
...
`b282216`, the merge of GRANT-EXPIRY-INTAKE (`fc49c35`), PROJECTION-SNAPSHOT
...
4,741 (events log, 2026-09-24T01:31:58Z). `d6787c5` added WORDING and
LIVE-PREP, and its joint gates passed with `pnpm test` 5,398 and 24. The
integration head, and the head these docs describe, is `c53aa25`: `d6787c5`
plus DOCS-6, SWEEP-2, SOL6-RUNTIME-FIX-2, SOL6-WEB-FIX, SOL6-AUTHORITY-FIX-2,
THERMO-2, PROTECTED-MIGRATIONS (the last migration is 0025),
OWNER-RULINGS-FIX and ARCH-4. Its joint gates passed with the counts in the
table (2026-09-24T03:45Z).
...
| `pnpm test`                                                                                                                                                                    | 5,465 passed, 24 skipped                                                                             | tested (merge trial, `d6787c5`)                                               |
| `tests/acceptance`                                                                                                                                                             | 3,852 passed, 16 skipped                                                                             | tested (merge trial, `d6787c5`)                                               |
| `db:conformance`                                                                                                                                                               | 125 named suites (118 invariant, 7 conformance), 4,789 of 4,789                                      | tested (merge trial, `d6787c5`)                                               |
...
settles the old 2,375 against 2,373: at `9221c29` the file ran 2,373 cells and
...
own work (EX-01, those three branches of `handlers.ts`): a fresh approved
reservation, or a lease the person's own pickup took. Until PROOF-CLOSURE
their 243 cells sent fabricated ids with no control and no retry.
...
failure that says nothing about the product. Run one file at a time, they
...
`app.fetch`. There is no network and no API server, and nothing below the
...
reason, and `identifier-negatives.test.ts`
...
**The proof is falsifiable, and a run showed it.** With the `assignee` case
routed through `task.assign`, the operation that writes the field, for the
demonstration, the database readback failed 33 cases
...
declared `task.cancel` and `task.restart` (9bf4c69), both cases run as plain
...
- **`lockTask` is not exported.** The proof hand-writes the predicate-less
  variant of the record lookup instead of taking it from the module it
  proves.
...
at `e5cbcba`. Each negative went red with its expectation inverted, and each
...
item 13 left unnamed. Tested in the lane run on `209eb29`.
...
The joint gates at `b282216` then passed, with `pnpm test` at 5,210 passed and
...
`158d6de`, at `d6787c5` and at `c53aa25`
...
- **Lease expiry and successor bounds, SOL6-RUNTIME-FIX-2.**
  ...
  a ceiling of 2 is accepted. Tested (merge trial, `faf3285`).
...
These three are open for Nathan. Nobody has decided them.

- **Two protected storage migrations.** SOL-RUNTIME-FIX proposed a trigger
  holding a cap and its envelope to one currency, and an aggregate cap-ceiling
  constraint trigger. Neither is written. Both are protected migrations and
  wait on Nathan's acceptance.
...
- **The heartbeat's bounds are unconfirmed.** `schedules-heartbeat.test.ts` now
  ...
  ([RUNTIME.md](RUNTIME.md#the-work-controls)). Root ruling 6 at `906613f`
