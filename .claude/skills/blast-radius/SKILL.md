...
Blast radius tells you what a change breaks somewhere else. (Upstream this line names two explainer skills that this repository does not vendor; the local edit is recorded in `.claude/skills/README.md`.)
...
1. Read the change. The diff, the symbols it adds, changes, and deletes, and what it now does differently, including the part the diff doesn't spell out. Pull the pull request and its commits with `gh pr view` and `git log`.
...
4. Be honest about each risk. Give it a real chance of happening and a real cost if it does. Keep the risks you confirmed; list the ones you checked and cleared separately. Same evidence rules as the rest of this repository. Cite a real `file:line`, a search that finds nothing is still an answer, and never make up a caller or an API.
...
6. For a big or wide change, get a second opinion from a frontier model at a different company and merge the answers; models from one vendor share blind spots, and different companies catch different real bugs. That is the adversarial pass in `AGENTS.md`, run by hand.
