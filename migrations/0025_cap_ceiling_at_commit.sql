...
-- through a larger total on its way to a valid one. It locks the cap row
-- before it sums, so of two committing transactions the second sums after the
-- first has committed and sees its rows. Every command path that raises a hold
-- already holds that lock (`locks.ts`, second in the order), so for them the
-- lock is one they have. The sum is `numeric` and exact at any `bigint` limit.
--
...
-- it without an execute grant. Grants are unchanged.
...
begin
  ...
  -- The lock first, as its own statement: the sum below then reads every
  -- envelope a transaction that held the lock before this one committed.
  select c.limit_minor into ceiling
